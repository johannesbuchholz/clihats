...
Array operand parsers are an exception to that rule. A missing optional array is set to an array of length zero unless specified differently using parameter `defaultValue`.
...
Unless specifically declaring a `defaultValue`, a missing optional array parameter is set to an array of length zero.
