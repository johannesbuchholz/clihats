...
=== Logging
During compilation, CliHats uses Slf4j with the simple provider and one single logger instance named `CliHats`. You may configure the log level used during processing by adding the JVM argument
----
